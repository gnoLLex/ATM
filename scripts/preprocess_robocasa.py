...
def collect_states_from_demo(h5_file, image_save_dir, demos_group, demo_k, view_names, track_model, task_emb, num_points, visualizer, save_vis=False):
    ...
    if "extra_states" not in root_grp:
        ...
        for robocasa_state_key, state_key in zip(EXTRA_STATES_ROBOCASA_KEYS, EXTRA_STATES_KEYS):
            if robocasa_state_key == "robot0_eef_pos":
                data = np.zeros((np.array(demos_group[demo_k]['obs'][robocasa_state_key]).shape[0], 3))
                continue
            elif robocasa_state_key == "robot0_eef_quat": # TODO: remove this we just filter on top here
                # convert quat to euler angle
                quat = np.array(demos_group[demo_k]['obs'][robocasa_state_key])
                # actually dont
                # we dont need this
                data = np.zeros((quat.shape[0], 3))
                continue
            elif robocasa_state_key == "robot0_eef_states":
                data = np.zeros((np.array(demos_group[demo_k]['obs']["robot0_eef_pos"]).shape[0], 6))
                continue
            else:
    ...
    for view in view_names:
        ...
        rgb = np.array(demos_group[demo_k]['obs'][robocasa_view])  # [T, H, W, 3], uint8
...
def generate_data(source_h5_path, target_dir, track_model, task_emb, skip_exist):
    ...
    with torch.no_grad():
        for idx in tqdm(range(len(demo_keys))):
            ...
            # try:
            #     collect_states_from_demo(h5_file_handle, image_save_dir, demos, demo_k, views, track_model, task_emb, num_points, visualizer, save_vis=False)#(idx%10==0))
            #     h5_file_handle.close()
            #     print(f"{save_path} is completed.")
