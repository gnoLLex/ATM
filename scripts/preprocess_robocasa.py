...
EXTRA_STATES_ROBOCASA_KEYS = ['robot0_gripper_qpos', 'robot0_joint_pos', 'robot0_eef_quat', 'robot0_eef_pos', 'robot0_eef_states']
EXTRA_STATES_KEYS = ['gripper_states', 'joint_states', 'ee_ori', 'ee_pos', 'ee_states']


def get_task_name_from_file_name(file_name):
    return "pick up something and put in on the stove"
...
def get_task_bert_embs(libero_root_dir):
    ...
    if not os.path.exists("robocasa/task_embedding_caches/task_emb_bert.npy"):
        ...
        os.makedirs("robocasa/task_embedding_caches/", exist_ok=True)
        np.save("robocasa/task_embedding_caches/task_emb_bert.npy", task_name_to_emb)
    else:
        task_name_to_emb = np.load("robocasa/task_embedding_caches/task_emb_bert.npy", allow_pickle=True).item()
    return task_name_to_emb

def online_tracking(track_model, video, points):
    pred_tracks, pred_vis = track_model(video_chunk=video[:, :track_model.step * 2], is_first_step=True, queries=points)

    for ind in range(0, video.shape[1] - track_model.step, track_model.step):
        chunk = video[:, ind : ind + track_model.step * 2]
        pred_tracks, pred_vis = track_model(video_chunk=chunk, queries=points)

    return pred_tracks, pred_vis

@torch.no_grad()
def offline_tracking(track_model, video, points):
    with torch.autocast(device_type="cuda", dtype=torch.half):
        pred_tracks, pred_vis = track_model(video=video, queries=points, backward_tracking=True)
    return pred_tracks, pred_vis

def track(track_model, video, points, use_online_tracking=False):
    if use_online_tracking:
        pred_tracks, pred_vis = online_tracking(track_model, video, points)
    else:
        pred_tracks, pred_vis = offline_tracking(track_model, video, points)

    return pred_tracks, pred_vis


def track_and_remove(tracker, video, points, var_threshold=10., use_online_tracking=False):
    B, T, C, H, W = video.shape
    pred_tracks, pred_vis = track(tracker, video, points, use_online_tracking) # [1, T, N, 2]
    ...
    # Track new points
    pred_tracks, pred_vis = track(tracker, video, new_points, use_online_tracking)

    return pred_tracks, pred_vis


def track_through_video(video, track_model, num_points=1000, use_online_tracking=False):
    T, C, H, W = video.shape
    ...
    pred_tracks, pred_vis = track_and_remove(track_model, video[None], points[None], use_online_tracking=use_online_tracking)
    pred_grid_tracks, pred_grid_vis = track_and_remove(track_model, video[None], grid_points[None], var_threshold=0., use_online_tracking=use_online_tracking)

    pred_tracks = torch.cat([pred_grid_tracks, pred_tracks], dim=2)
    pred_vis = torch.cat([pred_grid_vis, pred_vis], dim=2)
    return pred_tracks, pred_vis

def track_through_video_online(video, track_model, num_points=1000):
    """
    ...
    # Initialize online cotracker
    track_model(video_chunk=video[:, :track_model.step * 2], is_first_step=True, queries=grid_points[None])

    for ind in range(0, video.shape[1] - track_model.step, track_model.step):
        chunk = video[:, ind : ind + track_model.step * 2]
        pred_grid_tracks, pred_grid_vis = track_model(video_chunk=chunk)
...
def collect_states_from_demo(h5_file, image_save_dir, demos_group, demo_k, view_names, track_model, task_emb, num_points, visualizer, save_vis=False, use_online_tracking=False):
    ...
    if "actions" not in root_grp:
        root_grp.create_dataset("actions", data=actions[..., :7])  # only keep the first 7 dimensions

    if "extra_states" not in root_grp:
        extra_states_grp = root_grp.create_group("extra_states")
        for robocasa_state_key, state_key in zip(EXTRA_STATES_ROBOCASA_KEYS, EXTRA_STATES_KEYS):
            if robocasa_state_key in ["robot0_eef_pos", "robot0_eef_quat" ,"robot0_eef_states"]:
                continue

            data = np.array(demos_group[demo_k]['obs'][robocasa_state_key])
            extra_states_grp.create_dataset(state_key, data=data)
    ...
    for view in view_names:
        robocasa_view = view
        if robocasa_view == "agentview":
            robocasa_view = f"{robocasa_view}_left"
        robocasa_view = f"robot0_{robocasa_view}_image"
        rgb = np.array(demos_group[demo_k]['obs'][robocasa_view]).copy()  # [T, H, W, 3], uint8
        # rgb = rgb[:, ::-1, :, :].copy()  # The images in the raw Robocasa dataset is NOT upsidedown, so we need to dont flip it
        rgb = rearrange(rgb, "t h w c -> t c h w")
        T, C, H, W = rgb.shape

        pred_tracks, pred_vis = track_through_video(rgb, track_model, num_points=num_points, use_online_tracking=use_online_tracking)
        # pred_tracks, pred_vis = track_through_video_online(rgb, track_model, num_points=num_points)
...
def generate_data(source_h5_path, target_dir, track_model, task_emb, skip_exist, use_online_tracking):
    ...
    with torch.no_grad():
        for idx in tqdm(range(len(demo_keys))):
            ...
            collect_states_from_demo(h5_file_handle, image_save_dir, demos, demo_k, views, track_model, task_emb, num_points, visualizer, save_vis=(idx%10==0), use_online_tracking=use_online_tracking)
...
@click.command()
@click.option("--root", type=str, default="./data/robocasa/")
@click.option("--save", type=str, default="./data/atm_robocasa/")
@click.option("--suite", type=str, default="human_im")
@click.option("--skip_exist", type=bool, default=False)
@click.option("--use_online_tracking", type=bool, default=False)
def main(root, save, suite, skip_exist, use_online_tracking):
    """
    root: str, root directory of original libero dataset
    save: str, target directory to save the preprocessed data
    suite: str, the name of assigned suite, [human_im]
    ...
    if use_online_tracking:
        ...
        #cotracker = CoTrackerPredictor(checkpoint='./checkpoint/scaled_offline.pth')
    cotracker = cotracker.eval().cuda().float()
    count_parameters(cotracker)
    ...
    for source_h5 in os.listdir(suite_dir):
        ...
        generate_data(source_h5_path, save_dir, cotracker, task_bert_embs_dict[task_name], skip_exist, use_online_tracking)
